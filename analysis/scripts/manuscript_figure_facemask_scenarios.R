...
inf.testDay = read.csv('../output/output_facemask_scenarios.csv', stringsAsFactors = F)
inf.time = read.csv('../output/output_facemask_time_scenarios.csv', stringsAsFactors = F)
...
##==================================================#
outbreak_min = 100
jpeg('../figures/manuscript_figure_facemask_scenarios.jpeg',  width=6.5,height=4, units="in", res = 300)
...
for(pp_import in 1:length(importations_list)){
    ...
    br_prob = barplot(outbreak_prob_array, ylim = c(0,min(1,max(outbreak_prob_ci[,2])*1.2)), yaxt = 'n', xaxt = 'n')
    ...
    ## br_size = barplot(outbreak_size_array, yaxt = 'n', ylim = c(0,max(outbreak_size_array_high, na.rm = T)), xaxt = 'n')
    ...
    br_size = boxplot(infections ~ scenario,data = outbreak_out_box, yaxt = 'n', xaxt = 'n',
                      ylim = c(0,min(1200,max(outbreak_out_box$infections)*1.2)),
...
mtext("Compliance with face masks and physical distancing", side = 1, line = 0.9, outer = T)
...
##==================================================#
outbreak_min = 100
jpeg('../figures/manuscript_figure_facemask_scenarios_time.jpeg',  width=6.5,height=4, units="in", res = 300)
    ...
    for(cc in 1:length(fm_list)){
        ...
        for(nn in 1:nreps){
            ...
            if(nn == 1){
                ...
                if(pp_import == 1){
                    mtext(fm_list[cc], side = 3, line = 0.2)
                }
                if(cc == 1){
                    axis(2, las = 2)
                }
...
##==================================================#
outbreak_min = 100
jpeg('../figures/manuscript_figure_facemask_scenarios_time_curvedplots.jpeg',  width=6.5,height=4, units="in", res = 300)
layout(matrix(1:(length(importations_list) * length(fm_list)),nrow = length(importations_list), byrow = T))
par(mar = c(0,0,0,0), oma = c(5,5,3,3))
for(pp_import in 1:length(importations_list)){
    tmp_out = inf.time[inf.time$importation == importations_list[pp_import],]    
