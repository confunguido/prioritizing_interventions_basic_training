...
for(pp_import in 1:length(importations_list)){    
    ...
    for(ii_param in 1:length(fm_list)){
        ...
        ## Change values masks
        mask.final.compliance = fm_list[ii_param]
        compliance.final.avg = mask.protection * mask.final.compliance
        BOOL_compliance_time = 1
...
write.csv(inf.testDay, '../output/output_varying_facemask_scenarios.csv', row.names = F)
write.csv(inf.time, '../output/output_varying_facemask_time_scenarios.csv', row.names = F)
