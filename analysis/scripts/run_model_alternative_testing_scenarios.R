...
testdates_list = list(numeric(),numeric(),c(14),c(7,14),c(3,5))
testarrival_list = c(0,1,1,1,1)
test_type = list(c(),c(),c('pcr'),c('antigen','pcr'), c('antigen','antigen'))
...
for(pp_import in 1:length(importations_list)){
    ...
    for(ii_param in 1:length(testdates_list)){
        ...
        ## Change specificity values for this simulation
        pcr.spec.screen = 0.992
        pcr.spec.commercial = 0.992
...
## Reshape and save----------
##==================================================#
colnames(inf.testDay) = c(test_scenarios, c('rep','importation'))

colnames(inf.time) = c(test_scenarios, c('rep','importation'))
...
write.csv(inf.testDay, '../output/output_alternative_testing_scenarios.csv', row.names = F)
write.csv(inf.time, '../output/output_alternative_testing_time_scenarios.csv', row.names = F)
