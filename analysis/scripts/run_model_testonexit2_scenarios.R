...
for(pp_import in 1:length(importations_list)){
    ...
    for(ii_param in 1:length(testdates_list)){
        ...
        ## Change values
        BOOL_clinicalrelease = 0 ## TEST ON EXIT
...
write.csv(inf.testDay, '../output/output_testonexit2_scenarios.csv', row.names = F)
write.csv(inf.time, '../output/output_testonexit2_time_scenarios.csv', row.names = F)
